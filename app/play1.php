...
$zytimu = "#<ul class=\"list w-newfigure-list g-clear\">(.*?)</ul>#";
$bflist='#                                        <a data-daochu="(.*?)" class="btn js-site ea-site ea-site-(.*?)" href="(.*?)">(.*?)</a>#';
$jianjie = "#<p class=\"item-desc\">(.*?)<#";
$biaoti = "#<h1>(.*?)</h1>#";
$bofang = "#<a data-daochu=\"to=(.*?)\" class=\"btn js-site ea-site ea-site-(.*?)\" href=\"(.*?)\">(.*?)</a>#";
$tvdm = "#<a data-num=\"(.*?)\"data-daochu=\"to=(.*?)\" href=\"(.*?)\">#";
...
preg_match_all($tvdm, $tvinfo, $dmlist);
$mvsrc = $tvlist[3];
$bflu = $tvlist[1];
$bfyuan = $tvlist[4];
$jian = $jjarr[1][1];
...
<?php
if ($url_array[1] == "m") {
    foreach ($tvlist[3] as $key0 => $xianlu) {
        ?><a href='<?php echo $tvlist[3][$key0];?>' target='ajax'><?php echo $bfyuan[$key0];?></a><?php 
    }
} else {
    if ($url_array[1] == "tv") {
        ...
        if ($url_array[1] == "va") {
          ...
          <?php 
            ...
            if ($url_array[1] == "ct") {
                foreach ($dydm as $key1 => $md) {							
/*                     $mmm = ($much = $much + 1) + -1;
                    $kanji = "第'+jiid+'集"; */
                    ?><a href='<?php echo $dydm1[$key1];?>' target='ajax'>第<?php echo $md;?>集</a><?php
                }
            }
        }
    }
